...
class SystemActor extends Actor with ActorLogging with ActorSupport {
  ...
  def receive: Receive = {
    case Info =>
      sender ! nl.proja.pishake.model.SystemInfo(HardwareInfo(
        ...
        nl.proja.pishake.model.NetworkInfo(NetworkInfo.getHostname, NetworkInfo.getIPAddresses.toList, NetworkInfo.getFQDNs.toList, NetworkInfo.getNameservers.toList),
